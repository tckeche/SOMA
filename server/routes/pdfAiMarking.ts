...
export function registerPdfAiMarkingRoutes(app: Express): void {
  ...
  app.patch("/api/tutor/submission-uploads/:submissionId/ai-review/items/:itemId", requireTutor, async (req,res)=>{ const tutorId=(req as any).tutorId as string; const upload=await storage.getSubmissionUpload(Number(req.params.submissionId)); if(!upload) return res.status(404).json({message:"Submission not found"}); const quiz=await requireOwnedQuiz(upload.quizId,tutorId); if(!quiz) return res.status(403).json({message:"Access denied"}); const parsed=tutorReviewUpdateSchema.safeParse(req.body); if(!parsed.success) return res.status(400).json({message:"Invalid request"}); res.json(await storage.updatePdfMarkingReviewItem(Number(req.params.itemId),{...parsed.data,resolvedBy:tutorId,resolvedAt:new Date()} as any)); });
  ...
  app.post("/api/tutor/submission-uploads/:submissionId/ai-approve", requireTutor, async (req,res)=>{ const tutorId=(req as any).tutorId as string; const upload=await storage.getSubmissionUpload(Number(req.params.submissionId)); if(!upload) return res.status(404).json({message:"Submission not found"}); const quiz=await requireOwnedQuiz(upload.quizId,tutorId); if(!quiz) return res.status(403).json({message:"Access denied"}); const runs=await storage.getPdfMarkingRunsBySubmission(upload.id); const run=runs[0]; if(!run) return res.status(409).json({message:"No AI run to approve"}); const items=await storage.getPdfMarkingReviewItems(run.id); if(items.some((i)=>i.resolutionStatus==="pending")) return res.status(409).json({message:"Resolve all review items before approval"});
    ...
    const updated=await storage.markSubmissionUpload(upload.id,{score,maxScore,feedback:req.body?.feedback ?? "Approved AI-assisted PDF marking result.",status:"marked",markedAt:new Date(),aiMarkingStatus:"approved"}); res.json(stripStorage(updated!)); });
