...
// PDF/file-storage helpers (pdfUploadField, looksLikePdf, publicAttachment,
// publicSubmission, requireStorageConfigured, collectQuizStoragePaths,
// purgeStorageObjects) now live in server/modules/fileStorageAccess/service.ts
// and are consumed by the autoloaded pdfAttachments / pdfSubmissions modules.
// publicSubmission / collectQuizStoragePaths / purgeStorageObjects are imported
// above for the few inline routes that still reference them.
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // PDF attachment + submission upload/download routes now live in the
  // autoloaded pdfAttachments / pdfSubmissions modules. Only the tutor
  // "mark submission" route (POST /api/tutor/submission-uploads/:id/mark)
  // remains inline; it was never migrated and is registered further below.

  // Tutor "Regrade submissions" (TutorQuizReview) — recomputes completed
  // reports against the current question set and reports how many changed.
  // Gated to the quiz author. Not migrated to a domain module; stays inline.
  ...
  // Worksheet-attachment + submission upload/download routes (1)-(9) now
  // live in the autoloaded pdfAttachments / pdfSubmissions modules
  // (registered via registerDomainRoutes above). Storage helpers live in
  // server/modules/fileStorageAccess/service.ts. Only the tutor
  // "mark submission" route (10) below was never migrated and stays inline.
...
Return JSON object with fields: narrative, weaknesses, improvements, focusAreas, nextSteps`;
  ...
  app.post("/api/tutor/students/:studentId/ai/suggested-assessments", requireTutor, tutorAnalyticsAiLimiter, async (req, res) => {
    try {
      const tutorId = (req as any).tutorId;
      const studentId = String(req.params.studentId);
      const adopted = await storage.getAdoptedStudents(tutorId);
  ...
  app.post("/api/tutor/students/:studentId/ai/publish-suggested", requireTutor, tutorAnalyticsAiLimiter, async (req, res) => {
    try {
      const tutorId = (req as any).tutorId;
      const studentId = String(req.params.studentId);
  ...
  // Tutor quiz CRUD (create/get/update/detail), draft save/fetch, publish, and
  // manual question add/delete now live in the autoloaded domain modules
  // (tutorQuizzes, quizDrafts, quizPublish, questionManagement). They are
  // mounted ahead of these inline handlers via registerDomainRoutes() above.
