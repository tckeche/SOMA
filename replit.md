...
**Structured-output contract.** Every JSON-returning LLM call passes an `expectedSchema` to `generateWithFallback`, which the orchestrator (`aiOrchestrator.ts`) turns into provider-native structured output (Anthropic tool-use → guaranteed-valid JSON, OpenAI/DeepSeek/gpt-4o → `json_object` mode, Gemini → `responseSchema`). The tutor Co-Pilot chat and graph-retry calls use `COPILOT_RESPONSE_SCHEMA` (`routes.ts`). This is the primary defence against parse failures under provider fallback: when the primary model times out and the call falls back to Claude/Gemini, those models otherwise emit free-text JSON with unescaped inner quotes (e.g. IGCSE CS pseudocode `OUTPUT "x"`) that `JSON.parse` — and any post-hoc repair — cannot reliably recover, which previously produced "Generation failed or produced no valid draft". The `aiContracts` repair ladder is retained as defense-in-depth only, running after a strict parse fails. Gemini's converter rejects object schemas with empty `properties`, so schema array items list their common fields explicitly while leaving `additionalProperties` permissive so extra per-question fields (e.g. `graph_spec`) still pass through.
