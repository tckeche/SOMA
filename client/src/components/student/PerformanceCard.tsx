...
const TREND_META: Record<DashboardPerformance["recentTrend"], { Icon: typeof TrendingUp; tone: string; label: string }> = {
  ...
  flat: { Icon: Minus, tone: "text-slate-300", label: "Steady" },
...
export default function PerformanceCard({ performance, subjects }: Props) {
  ...
  return (
    <section
      className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900/95 to-slate-900/60 p-6 shadow-xl"
      aria-label="Performance"
      data-testid="panel-performance"
    >
      <header className="flex items-start justify-between gap-4 mb-5">
        <div>
          <p className="text-[10px] uppercase tracking-wider text-slate-500">Your performance, today</p>
          <h2 className="text-xl font-semibold text-slate-100 mt-1 flex items-center gap-2">
            <Target className="w-5 h-5 text-violet-400" />
            How you're doing
          </h2>
        </div>
        <div className={`inline-flex items-center gap-1.5 text-xs ${trendMeta.tone} px-3 py-1.5 rounded-full bg-slate-900/60 border border-slate-800`}>
          <TrendIcon className="w-3.5 h-3.5" />
          {trendMeta.label}
        </div>
      </header>

      <p className="text-sm text-slate-200 leading-relaxed" data-testid="text-performance-message">
        {performance.message}
      </p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
          ...
          <p className="text-2xl font-semibold text-slate-100">
            {performance.totalCompleted}<span className="text-sm text-slate-500"> / {performance.totalAssigned}</span>
          </p>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-3">
          ...
          <p className="text-base font-semibold text-slate-100 truncate" title={performance.bestSubject ?? undefined}>
      ...
      {ranked.length > 0 && (
        <div className="mt-5">
          <p className="text-[10px] uppercase tracking-wider text-slate-500 mb-2">By subject</p>
