...
vi.mock("../server/services/aiPipeline", () => ({
  ...
  runQuestionAudit: vi.fn(async (questions: any[]) => ({
    questions: questions.map((q) => ({
      ...
      explanation: q.explanation || "Mock explanation.",
      marks: q.marks,
...
// matching the default SUPER_ADMIN_EMAIL fallback). A super_admin can read ANY
// soma quiz, so this is the credential used to exercise the (now authed) soma
// read endpoints regardless of who authored the quiz.
const TEST_SUPER_ADMIN_UUID = "ffffffff-2222-3333-4444-666666666666";
...
// ─── QUIZ: Soma read endpoints (authenticated) ────────────────────────────────
describe("GET /api/soma/quizzes", () => {
  it("returns 401 without authentication (protected endpoint)", async () => {
    const res = await request.get("/api/soma/quizzes");
    expect(res.status).toBe(401);
  });

  it("returns 200 with array of quizzes for an authenticated super admin", async () => {
...
describe("GET /api/soma/quizzes/:id", () => {
  it("returns 404 for non-existent quiz (authenticated)", async () => {
    const token = await getSuperAdminToken();
    const res = await request.get("/api/soma/quizzes/99999").set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(404);
  });

  it("returns quiz data for existing quiz (authenticated)", async () => {
    const cookie = await loginAsAdmin();
    const token = await getSuperAdminToken();
    const quiz = await createTestQuiz(cookie, { title: "Authed Quiz" });
    const res = await request.get(`/api/soma/quizzes/${quiz.id}`).set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(res.body.title).toBe("Authed Quiz");
  });
});
...
describe("Admin Quiz CRUD", () => {
  ...
  it("DELETE /api/tutor/quizzes/:id removes quiz", async () => {
    ...
    const getRes = await request.get(`/api/soma/quizzes/${quiz.id}`).set("Authorization", `Bearer ${superToken}`);
...
describe("GET /api/soma/quizzes (generated, authenticated)", () => {
  it("returns 200 with array for an authenticated super admin", async () => {
    const token = await getSuperAdminToken();
    const res = await request.get("/api/soma/quizzes").set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body)).toBe(true);
...
describe("GET /api/soma/quizzes/:id (generated, authenticated)", () => {
...
describe("GET /api/soma/quizzes/:id/questions (generated, authenticated)", () => {
...
describe("Cascade delete behavior", () => {
  ...
  it("deleting a quiz removes its questions", async () => {
    ...
    await addQuestions(cookie, quiz.id);
    const superToken = await getSuperAdminToken();
    // Verify questions exist via the (authenticated) soma read endpoint
    const qRes = await request.get(`/api/soma/quizzes/${quiz.id}/questions`).set("Authorization", `Bearer ${superToken}`);
